...
import { addSeason, addUnderDogs, addWinners, getFinalPick } from '../../actions/index'
...
export class PickMatchUp extends Component {
  ...
  generatePick = (filteredTeams) => {
      let randomIndex = Math.round(Math.random() * (filteredTeams.length - 0)) + 0;
      ...
      <section className="PickMatchUp">
        ...
        <div id="pickUnderDog-buttons-container">
          <Link to="/yourTeam" id="yourTeam-link">
            <input type="submit" id="gloves" value="" onClick={this.filterUnderDogs}/>
            <button id="gloves-button" onClick={this.filterUnderDogs}>Absolutely!</button>
          </Link>
        </div>
        <div id="pickSafeBet-buttons-container">
          <Link to="/yourTeam" id="yourTeam-link">
            <input type="submit" id="trophy" value="" onClick={() => this.filterWinningestTeams()}/>
            <button id="trophy-button" onClick={() => this.filterWinningestTeams()}>No, I Prefer A Reliable Win</button>
          </Link>
        </div>
      </section>
    )
